...
INPUT_FILE = "good_repro1_translated.json"
OUTPUT_FILE = "good_repro1_cross_op.json"
...
def process_entries(data):
    for entry in data:
        ...
        # Run Node command
        node_section = entry.get("RUN_op_node")
        if node_section and "combined_command_node" in node_section:
            cmd = node_section["combined_command_node"]
            cwd = os.path.join(NODE_BASE, issue_number)

            if os.path.isdir(cwd):
                print(f"[NODE] Running issue {issue_number}...")
                result = run(cmd, cwd)
            else:
                print(f"[NODE] Directory missing: {cwd}")
                result = {
                    "stdout": "",
        ...
        # Run Bun command
        bun_section = entry.get("RUN_op_bun")
        if bun_section and "combined_command_bun" in bun_section:
            cmd = bun_section["combined_command_bun"]
            cwd = os.path.join(BUN_BASE, issue_number)

            if os.path.isdir(cwd):
                print(f"[BUN] Running issue {issue_number}...")
                result = run(cmd, cwd)
            else:
                print(f"[BUN] Directory missing: {cwd}")
                result = {
                    "stdout": "",
                    "stderr": f"Directory not found: {cwd}",
                    "return_code": -1
