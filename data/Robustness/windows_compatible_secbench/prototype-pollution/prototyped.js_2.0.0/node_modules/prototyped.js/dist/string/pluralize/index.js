 ...
 * @return {RegExp}
 */
const sanitizeRule = (rule) => isString_1.default(rule) ? new RegExp(`^${rule}$`, "i") : rule;
/**
...
const replaceWord = (replaceMap, keepMap, rules) => (word) => {
    ...
    if (hasOwnProperty.call(keepMap, token))
    ...
    if (!token.length || hasOwnProperty.call(uncountables, token))
        return word;
 ...
 */
const addPluralRule = (rule, replacement) => pluralRules.push([sanitizeRule(rule), replacement]);
/**
 ...
 * @param {String} replacement
 */
const addSingularRule = (rule, replacement) => singularRules.push([sanitizeRule(rule), replacement]);
